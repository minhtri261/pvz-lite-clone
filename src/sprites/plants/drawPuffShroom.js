'use strict';
function _drawPuffShroomBody(ctx, animTime, shootT) {
    ...
    cG.addColorStop(0,    '#EA80FC');
    cG.addColorStop(0.35, '#9C27B0');
    cG.addColorStop(0.8,  '#6A1B9A');
    cG.addColorStop(1,    '#4A148C');
...
function drawPuffShroom(ctx, x, y, animTime, shootT, stackCount = 1) {
    if (stackCount === 2) {
        // Con trái (còn sống sau cùng) — vẽ trước để nằm phía sau
        ctx.save();
        ...
        ctx.scale(0.76, 0.76);
        _drawPuffShroomBody(ctx, animTime * 1.08 + 0.5, shootT);
        ctx.restore();
    } else {
        ctx.save();
