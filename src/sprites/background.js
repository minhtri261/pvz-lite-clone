...
// theme = 'day' | 'night' | 'egypt' | 'egypt-night'
function drawBackground(ctx, activeRows, hoverCol, hoverRow, theme = 'day') {
    const isNight     = theme === 'night' || theme === 'egypt-night';
    const isEgypt     = theme === 'egypt' || theme === 'egypt-night';
    const isEgyptNight = theme === 'egypt-night';

    // ── Màu cỏ theo chủ đề ────────────────────────────────────
    const LAWN_BRIGHT = isEgyptNight
        ? ['#5c4420', '#503a1a', '#5c4420', '#503a1a', '#5c4420']
        : isEgypt
            ? ['#c8a038', '#bc9030', '#c8a038', '#bc9030', '#c8a038']
            : isNight
                ? ['#2c4e32', '#264828', '#2c4e32', '#264828', '#2c4e32']
                : ['#5cba58', '#52aa4e', '#5cba58', '#52aa4e', '#5cba58'];
    const LAWN_DARK = isEgyptNight
        ? ['#362810', '#2e220c', '#362810', '#2e220c', '#362810']
        : isEgypt
    ...
    if (isNight) {
        ...
        if (isEgypt) {
            sky.addColorStop(0, '#160d08');
            sky.addColorStop(1, '#3c2614');
        } else {
    ...
    // Kim tự tháp bóng ở phía xa — trang trí, hiện ở mọi theme Egypt (ngày lẫn đêm)
    if (isEgypt) {
        ctx.fillStyle = isNight ? 'rgba(15,9,4,0.55)' : 'rgba(200,140,40,0.35)';
        ctx.beginPath();
        ctx.moveTo(W - 160, GY + 6);
        ctx.lineTo(W - 120, GY - 45);
        ctx.lineTo(W - 80, GY + 6);
        ctx.closePath(); ctx.fill();
        ctx.fillStyle = isNight ? 'rgba(22,13,6,0.4)' : 'rgba(210,150,50,0.22)';
        ctx.beginPath();
    ...
    // ── Tường nhà ─────────────────────────────────────────────
    const wallTop = isEgyptNight ? '#4a3210' : isEgypt ? '#d4a850' : isNight ? '#7a5a20' : '#cc9e58';
    const wallBot = isEgyptNight ? '#2c1c06' : isEgypt ? '#a87830' : isNight ? '#5a3e10' : '#a87838';
    ...
    const brickLine = isEgyptNight ? 'rgba(20,10,0,0.55)' : isEgypt ? 'rgba(100,60,0,0.38)' : isNight ? 'rgba(40,20,0,0.45)' : 'rgba(70,35,5,0.42)';
    const brickFill = isEgyptNight ? 'rgba(90,55,10,0.06)' : isEgypt ? 'rgba(220,160,50,0.07)' : isNight ? 'rgba(100,65,15,0.07)' : 'rgba(200,140,60,0.07)';
    const bH = 17, bW = 25;
    ...
    postG.addColorStop(0, isEgyptNight ? 'rgba(15,8,0,0.85)' : isEgypt ? 'rgba(80,40,0,0.8)' : isNight ? 'rgba(20,10,0,0.8)' : 'rgba(55,25,0,0.75)');
    postG.addColorStop(0.6, isEgyptNight ? 'rgba(35,20,0,0.6)' : isEgypt ? 'rgba(130,70,10,0.55)' : isNight ? 'rgba(50,28,0,0.55)' : 'rgba(100,55,10,0.5)');
    postG.addColorStop(1, 'rgba(0,0,0,0.05)');
    ctx.fillStyle = postG; ctx.fillRect(GX - 14, GY, 14, GRID_H);

    // Cửa nhà
    const doorY = GY + GRID_H - 118;
    ctx.fillStyle = isEgyptNight ? '#2c1404' : isEgypt ? '#5a2a08' : isNight ? '#4a1e08' : '#6a2e0c';
    rr(ctx, 16, doorY, 58, 112, 8); ctx.fill();
    ctx.strokeStyle = isEgyptNight ? '#180a00' : isEgypt ? '#3a1400' : isNight ? '#28100000' : '#3e1600'; ctx.lineWidth = 3; ctx.stroke();
    ctx.fillStyle = isEgyptNight ? 'rgba(0,0,0,0.45)' : isEgypt ? 'rgba(0,0,0,0.25)' : isNight ? 'rgba(0,0,0,0.35)' : 'rgba(0,0,0,0.18)';
    ...
    // Cửa sổ
    const winY = GY + 24;
    ctx.fillStyle = isEgyptNight ? '#c8902c' : isEgypt ? '#ffd060' : isNight ? '#f0d060' : '#b0d8f0';
    rr(ctx, 22, winY, 50, 42, 4); ctx.fill();
    ctx.strokeStyle = isEgyptNight ? '#3a2200' : isEgypt ? '#7a4a00' : isNight ? '#4a2800' : '#5a2800'; ctx.lineWidth = 2.5; ctx.stroke();
    if (isNight) {
        const glow = ctx.createRadialGradient(47, winY + 21, 2, 47, winY + 21, isEgypt ? 45 : 50);
        glow.addColorStop(0, isEgypt ? 'rgba(220,170,60,0.16)' : 'rgba(240,200,60,0.18)');
        glow.addColorStop(1, isEgypt ? 'rgba(220,170,60,0)' : 'rgba(240,200,60,0)');
    ...
    ctx.strokeStyle = isEgyptNight ? 'rgba(80,50,0,0.55)' : isEgypt ? 'rgba(150,90,0,0.55)' : isNight ? 'rgba(120,80,0,0.55)' : 'rgba(90,40,0,0.55)';
    ctx.lineWidth = 1.5;
    ...
    for (let row = 0; row < ROWS; row++) {
        ...
        const rowY = GY + row * CELL_H;
        ctx.fillStyle = isEgyptNight ? 'rgba(10,4,0,0.78)' : isEgypt ? 'rgba(60,30,0,0.68)' : isNight ? 'rgba(0,0,0,0.72)' : 'rgba(20,9,2,0.65)';
        ctx.fillRect(GX, rowY, GRID_W, CELL_H);
        ctx.strokeStyle = isEgyptNight ? 'rgba(40,20,0,0.22)' : isEgypt ? 'rgba(120,70,0,0.25)' : isNight ? 'rgba(60,30,0,0.22)' : 'rgba(95,50,8,0.22)';
        ctx.lineWidth = 1;
    ...
    // Dải phải và đáy
    ctx.fillStyle = isEgyptNight ? '#1c1006' : isEgypt ? '#c8880a' : isNight ? '#1a3020' : '#3a7a20';
    ctx.fillRect(GRID_RIGHT, GY, W - GRID_RIGHT, GRID_H);
    const bottomG = ctx.createLinearGradient(0, GRID_BOT, 0, H);
    bottomG.addColorStop(0, isEgyptNight ? '#241404' : isEgypt ? '#a06808' : isNight ? '#162a12' : '#2e5a18');
    bottomG.addColorStop(1, isEgyptNight ? '#0e0800' : isEgypt ? '#704000' : isNight ? '#0a1808' : '#1a3408');
    ctx.fillStyle = bottomG; ctx.fillRect(0, GRID_BOT, W, H - GRID_BOT);

    // ── Đường lưới ────────────────────────────────────────────
    ctx.strokeStyle = isEgyptNight ? 'rgba(40,20,0,0.30)' : isEgypt ? 'rgba(100,60,0,0.30)' : isNight ? 'rgba(0,60,20,0.30)' : 'rgba(0,45,0,0.28)';
    ctx.lineWidth = 1;
    ...
    if (isEgypt) {
        ...
        for (let row = 0; row < ROWS; row++) {
            ...
            for (let col = 0; col < COLS; col++) {
                ...
                for (const bl of GRASS_BLADES[row][col]) {
                    ...
                    const g = isEgyptNight ? 55 + shade * 0.6 : 140 + shade;
                    ctx.strokeStyle = isEgyptNight
                        ? `rgba(${Math.min(255, g + 30)},${g},${Math.max(0, g - 40)},0.4)`
                        : `rgba(${g + 50},${g},${Math.max(0, g - 60)},0.55)`;
                    ctx.lineWidth = 1.5;
                    ctx.beginPath();
        ...
        ctx.fillStyle = isEgyptNight ? 'rgba(120,90,30,0.1)' : 'rgba(210,160,40,0.18)';
